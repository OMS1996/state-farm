...
def create_preprocessors(
    data_source: Union[
        str, pd.DataFrame
    ] = "https://raw.githubusercontent.com/OMS1996/state-farm/main/data/exercise_26_train.csv"
):
    ...
    train_imputed = pd.DataFrame(
        numeric_imputer.transform(train_df[numeric_cols]), columns=numeric_cols
    )
...
def preprocess_data(
    ...
    std_scaler: StandardScaler,
    selected_features: List = VARIABLES,
) -> pd.DataFrame:
    """
    ...
    for var in vars:
        if var in data.columns:
            dummies = pd.get_dummies(
                data[var], drop_first=True, prefix=var, prefix_sep="_", dummy_na=True
            )
...
def run_preprocess(input_data: Union[str, Dict, pd.DataFrame, pd.Series]):
    ...
    if os.path.exists("numeric_imputer.pkl") and os.path.exists("std_scaler.pkl"):
